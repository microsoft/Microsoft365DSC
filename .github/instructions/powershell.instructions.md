...
- **Type Selection:**
  ...
  - Use the full type declaration (e.g., use [System.String] instead of [string])
  - Write the Parameter Type on a different line than the parameter name
  - If a parameter is Mandatory, use `Mandatory = $true` instead of short `Mandatory`
...
- **Output Objects:**
  ...
  - Use Hashtable for structured data. If required, use an OrderedHashtable.
  - Avoid Write-Host for data output. Use `Write-M365DSCHost` for console output purposes or `Write-Verbose` for logging.
    Otherwise, stick to directly outputting the object using its variable name, without an additional call to `Write-Output`.
...
function Update-ResourceStatus {
    ...
    process {
        ...
        # If the resource is used in a display scenario, use [ordered]
        $resource = @{
            Name        = $Name
            Status      = $Status
...
- **Comment-Based Help:** Include comment-based help for any public-facing function or cmdlet. Outside of the function, add a `<# ... #>` help comment with at least:
...
- **Consistent Formatting:**
  ...
  - Opening braces on a new line after the statement
