...
function Set-TargetResource
{
    ...
    {
        Write-Verbose -Message "Creating a Teams Tenant Trusted IP Address with Identity {$Identity}"
        $CreateParameters = Remove-M365DSCAuthenticationParameter -BoundParameters $PSBoundParameters
        New-CsTenantTrustedIPAddress @CreateParameters | Out-Null
    }
    elseif ($Ensure -eq 'Present' -and $currentInstance.Ensure -eq 'Present')
    {
        Write-Verbose -Message "Updating the Teams Tenant Trusted IP Address with Identity {$Identity}"
        $UpdateParameters = Remove-M365DSCAuthenticationParameter -BoundParameters $PSBoundParameters
        Set-CsTenantTrustedIPAddress @UpdateParameters | Out-Null
    }
    elseif ($Ensure -eq 'Absent' -and $currentInstance.Ensure -eq 'Present')
    {
        Write-Verbose -Message "Removing the Teams Tenant Trusted IP Address with Identity {$Identity}"
...
function Export-TargetResource
{
    ...
    try
    {
        [array]$getValue = Get-CsTenantTrustedIPAddress -Filter $Filter -ErrorAction Stop
