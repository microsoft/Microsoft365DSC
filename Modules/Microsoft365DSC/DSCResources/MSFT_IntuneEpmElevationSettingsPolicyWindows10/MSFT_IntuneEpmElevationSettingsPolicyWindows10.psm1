...
function Get-TargetResource
{
    ...
    Write-Verbose -Message "Getting configuration for the Intune Endpoint Privilege Management Elevation Settings Policy for Windows10 with Id {$Id} and Name {$DisplayName}"
    ...
    try
    {
        if (-not $Script:exportedInstance -or $Script:exportedInstance.DisplayName -ne $DisplayName)
        {
            ...
            if ($null -eq $getValue)
            {
                Write-Verbose -Message "Could not find an Intune Endpoint Privilege Management Elevation Settings Policy for Windows10 with Id {$Id}"
            ...
            if ($null -eq $getValue)
            {
                Write-Verbose -Message "Could not find an Intune Endpoint Privilege Management Elevation Settings Policy for Windows10 with Name {$DisplayName}."
                return $nullResult
            }
        }
        else
        ...
        $Id = $getValue.Id
        Write-Verbose -Message "An Intune Endpoint Privilege Management Elevation Settings Policy for Windows10 with Id {$Id} and Name {$DisplayName} was found"
...
function Set-TargetResource
{
    ...
    Write-Verbose -Message "Setting configuration of the Intune Endpoint Privilege Management Elevation Settings Policy for Windows10 with Id {$Id} and Name {$DisplayName}"
    ...
    $templateReferenceId = 'e7dcaba4-959b-46ed-88f0-16ba39b14fd8_1'
    $platforms = 'windows10'
    ...
    {
        Write-Verbose -Message "Removing the Intune Endpoint Privilege Management Elevation Settings Policy for Windows10 with Id {$($currentInstance.Id)}"
        #region resource generator code
