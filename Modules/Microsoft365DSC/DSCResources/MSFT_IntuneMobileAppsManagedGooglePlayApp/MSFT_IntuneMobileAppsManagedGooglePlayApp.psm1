Confirm-M365DSCModuleDependency -ModuleName 'MSFT_IntuneMobileAppsManagedGooglePlayApp'
...
function Get-TargetResource
{
    ...
    Write-Verbose -Message "Getting configuration for the Intune Mobile Apps Managed Google Play App with Id {$Id} and DisplayName {$DisplayName}"
    ...
    try
    {
        if (-not $Script:exportedInstance -or $Script:exportedInstance.DisplayName -ne $DisplayName)
        {
            ...
            if ($null -eq $getValue)
            {
                Write-Verbose -Message "Could not find an Intune Mobile Apps Managed Google Play App with Id {$Id}"
            ...
            if ($null -eq $getValue)
            {
                Write-Verbose -Message "Could not find an Intune Mobile Apps Managed Google Play App with DisplayName {$DisplayName}."
                return $nullResult
            }
        ...
        $Id = $getValue.Id
        Write-Verbose -Message "An Intune Mobile Apps Managed Google Play App with Id {$Id} and DisplayName {$DisplayName} was found"
...
function Set-TargetResource
{
    ...
    Write-Verbose -Message "Setting configuration of the Intune Mobile Apps Managed Google Play App with Id {$Id} and DisplayName {$DisplayName}"
