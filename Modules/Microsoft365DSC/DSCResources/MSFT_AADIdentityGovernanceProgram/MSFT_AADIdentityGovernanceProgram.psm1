function Get-TargetResource
{
    ...
    try
    {
        ...
        if ($null -eq $getValue)
        {
            Write-Verbose -Message "Could not find an Azure AD Identity Governance Program with DisplayName {$DisplayName}."
            return $nullResult
        }
        $Id = $getValue.Id
...
function Set-TargetResource
{
    ...
    {
        Write-Verbose -Message "Creating an Azure AD Identity Governance Program with DisplayName {$DisplayName}"

        $createParameters = ([Hashtable]$BoundParameters).Clone()
        $createParameters.Remove('Id') | Out-Null

        New-MgBetaProgram -BodyParameter $createParameters
    }
    elseif ($Ensure -eq 'Present' -and $currentInstance.Ensure -eq 'Present')
    {
        Write-Verbose -Message "Updating the Azure AD Identity Governance Program with Id {$($currentInstance.Id)}"

        $updateParameters = ([Hashtable]$BoundParameters).Clone()
        $updateParameters.Remove('Id') | Out-Null

        Update-MgBetaProgram `
            -ProgramId $currentInstance.Id `
            -BodyParameter $UpdateParameters
    }
    elseif ($Ensure -eq 'Absent' -and $currentInstance.Ensure -eq 'Present')
    {
        Write-Verbose -Message "Removing the Azure AD Identity Governance Program with Id {$($currentInstance.Id)}"
        Remove-MgBetaProgram -ProgramId $currentInstance.Id
    }
}
...
function Export-TargetResource
{
    ...
    try
    {
        ...
        [array]$getValue = Get-MgBetaProgram `
            -Filter $Filter `
            -All `
            -ErrorAction Stop
        #endregion
