function Get-TargetResource
{
    ...
    try
    {
        ...
        if ($null -eq $getValue)
        {
            Write-Verbose -Message "Could not find an Intune Device Configuration Edition Upgrade Policy for Windows10 with Id {$Id}"
        ...
        if ($null -eq $getValue)
        {
            Write-Verbose -Message "Could not find an Intune Device Configuration Edition Upgrade Policy for Windows10 with DisplayName {$DisplayName}"
            return $nullResult
        }
        $Id = $getValue.Id
        Write-Verbose -Message "An Intune Device Configuration Edition Upgrade Policy for Windows10 with Id {$Id} and DisplayName {$DisplayName} was found."

        #region resource generator code
        $enumLicenseType = $null
        if ($null -ne $getValue.AdditionalProperties.licenseType)
        {
            $enumLicenseType = $getValue.AdditionalProperties.licenseType.ToString()
        }

        $enumTargetEdition = $null
        if ($null -ne $getValue.AdditionalProperties.targetEdition)
        {
            $enumTargetEdition = $getValue.AdditionalProperties.targetEdition.ToString()
        }

        $enumWindowsSMode = $null
        if ($null -ne $getValue.AdditionalProperties.windowsSMode)
        {
            $enumWindowsSMode = $getValue.AdditionalProperties.windowsSMode.ToString()
        }
        #endregion

        $results = @{
            #region resource generator code
            License               = $getValue.AdditionalProperties.license
            LicenseType           = $enumLicenseType
            ProductKey            = $getValue.AdditionalProperties.productKey
            TargetEdition         = $enumTargetEdition
...
function Test-TargetResource
{
    ...
    Write-Verbose -Message "Testing configuration of the Intune Device Configuration Edition Upgrade Policy for Windows10 with Id {$Id} and DisplayName {$DisplayName}"
    ...
    #Checking last 4 digits of the productkey as obfuscated by Intune
    $ValuesToCheck.Remove('ProductKey') | Out-Null
    if ($ProductKey -ne $CurrentValues.ProductKey)
    {
        if ([String]::IsNullOrWhiteSpace($ProductKey) -xor [String]::IsNullOrWhiteSpace($CurrentValues.ProductKey))
        {
            $testResult = $false
        }
