...
function Get-TargetResource
{
    ...
    try
    {
        if (-not $Script:exportedInstance -or $Script:exportedInstance.Identity -ne $Identity)
        {
            ...
            $instance = Get-CsTeamsCortanaPolicy -Identity $Identity -ErrorAction SilentlyContinue
        }
        else
        {
...
function Set-TargetResource
{
    ...
    Write-Verbose -Message "Setting configuration for TeamsComplianceRecordingPolicy $Identity"
    ...
    if ($Ensure -eq 'Present' -and $currentInstance.Ensure -eq 'Absent')
    {
        ...
        Write-Verbose -Message "Creating a Teams Cortana Policy {$Identity} with Parameters:`r`n$(Convert-M365DscHashtableToString -Hashtable $CreateParameters)"
        New-CsTeamsCortanaPolicy @CreateParameters | Out-Null
    }
    elseif ($Ensure -eq 'Present' -and $currentInstance.Ensure -eq 'Present')
    {
        $UpdateParameters = Remove-M365DSCAuthenticationParameter -BoundParameters $PSBoundParameters
        Write-Verbose -Message "Updating the Teams Cortana Policy with Identity {$Identity}"
        Set-CsTeamsCortanaPolicy @UpdateParameters | Out-Null
    }
    elseif ($Ensure -eq 'Absent' -and $currentInstance.Ensure -eq 'Present')
    {
        Write-Verbose -Message "Removing the Teams Cortana Policy with Identity {$Identity}"
...
function Export-TargetResource
{
    ...
    try
    {
        [array]$getValue = Get-CsTeamsCortanaPolicy -Filter $Filter -ErrorAction Stop
