...
function Get-TargetResource
{
    ...
    try
    {
        if (-not $Script:exportedInstance -or $Script:exportedInstance.Identity -ne $Identity)
        {
            ...
            $instance = Get-CsTenantNetworkSite -Identity $Identity -ErrorAction SilentlyContinue
        }
        else
        {
...
function Set-TargetResource
{
    ...
    {
        Write-Verbose -Message "Creating a Teams Tenant Network Site with Identity {$Identity}"
        $CreateParameters = Remove-M365DSCAuthenticationParameter -BoundParameters $PSBoundParameters
        New-CsTenantNetworkSite @CreateParameters | Out-Null
    }
    elseif ($Ensure -eq 'Present' -and $currentInstance.Ensure -eq 'Present')
    {
        Write-Verbose -Message "Updating the Teams Tenant Network Site with Identity {$Identity}"
        $UpdateParameters = Remove-M365DSCAuthenticationParameter -BoundParameters $PSBoundParameters
        Set-CsTenantNetworkSite @UpdateParameters | Out-Null
    }
    elseif ($Ensure -eq 'Absent' -and $currentInstance.Ensure -eq 'Present')
    {
        Write-Verbose -Message "Removing the Teams Tenant Network Site with Identity {$Identity}"
...
function Export-TargetResource
{
    ...
    try
    {
        [array]$getValue = Get-CsTenantNetworkSite -Filter $Filter -ErrorAction Stop
