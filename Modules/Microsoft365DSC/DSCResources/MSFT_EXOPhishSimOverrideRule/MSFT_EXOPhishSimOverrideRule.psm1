Confirm-M365DSCModuleDependency -ModuleName 'MSFT_EXOPhishSimOverrideRule'
...
function Get-TargetResource
{
    ...
    Write-Verbose -Message "Getting configuration for Phishing Simulation Override Rule with Identity {$Identity}"
    ...
    try
    {
        if (-not $Script:exportedInstance -or $Script:exportedInstance.Identity -ne $Identity)
        {
            ...
            if ($null -eq $instance)
            {
                Write-Verbose -Message "Phishing Simulation Override Rule with Identity {$Identity} not found"
                return $nullResult
            }
        }
        else
        ...
        Write-Verbose -Message "Found Phishing Simulation Override Rule with Identity {$Identity}"
...
function Set-TargetResource
{
    ...
    Write-Verbose -Message "Setting configuration for Phishing Simulation Override Rule with Identity {$Identity}"
