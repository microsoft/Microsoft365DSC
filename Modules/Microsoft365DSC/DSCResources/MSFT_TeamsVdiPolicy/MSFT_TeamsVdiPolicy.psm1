...
function Set-TargetResource
{
    ...
    {
        Write-Verbose -Message "Updating the Teams VDI Policy with Identity {$Identity}"
        $UpdateParameters = Remove-M365DSCAuthenticationParameter -BoundParameters $PSBoundParameters
        Set-CsTeamsVdiPolicy @UpdateParameters | Out-Null
    }
    elseif ($Ensure -eq 'Absent' -and $currentInstance.Ensure -eq 'Present')
    {
        Write-Verbose -Message "Removing the Teams VDI Policy with Identity {$Identity}"
...
function Export-TargetResource
{
    ...
    try
    {
        [array]$getValue = Get-CsTeamsVdiPolicy -Filter $Filter -ErrorAction Stop
