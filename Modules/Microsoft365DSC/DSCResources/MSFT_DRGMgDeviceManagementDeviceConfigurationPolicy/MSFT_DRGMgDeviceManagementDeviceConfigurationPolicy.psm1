function Get-TargetResource
{
    ...
    try
    {
        ...
        if ($null -eq $getValue)
        {
            Write-Verbose -Message "Could not find an D R G Mg Device Management Device Configuration Policy with Id {$Id}"
        ...
        if ($null -eq $getValue)
        {
            Write-Verbose -Message "Could not find an D R G Mg Device Management Device Configuration Policy with DisplayName {$DisplayName}"
            return $nullResult
        }
        $Id = $getValue.Id
        Write-Verbose -Message "An D R G Mg Device Management Device Configuration Policy with Id {$Id} and DisplayName {$DisplayName} was found."
        #region resource generator code
        $complexRolloutSettings = @{}
        $complexRolloutSettings.Add('OfferEndDateTimeInUTC', $getValue.offerEndDateTimeInUTC)
        $complexRolloutSettings.Add('OfferIntervalInDays', $getValue.offerIntervalInDays)
        $complexRolloutSettings.Add('OfferStartDateTimeInUTC', $getValue.offerStartDateTimeInUTC)
        ...
        $results = @{
            #region resource generator code
            DeployableContentDisplayName = $getValue.DeployableContentDisplayName
            Description                  = $getValue.Description
            DisplayName                  = $getValue.DisplayName
            EndOfSupportDate             = $getValue.EndOfSupportDate
...
function Set-TargetResource
{
    ...
    {
        Write-Verbose -Message "Creating an D R G Mg Device Management Device Configuration Policy with DisplayName {$DisplayName}"
    ...
    {
        Write-Verbose -Message "Updating the D R G Mg Device Management Device Configuration Policy with Id {$($currentInstance.Id)}"
    ...
    {
        Write-Verbose -Message "Removing the D R G Mg Device Management Device Configuration Policy with Id {$($currentInstance.Id)}"
        #region resource generator code
...
function Test-TargetResource
{
    ...
    Write-Verbose -Message "Testing configuration of the D R G Mg Device Management Device Configuration Policy with Id {$Id} and DisplayName {$DisplayName}"
    ...
    $dateProperties=($ValuesToCheck.getEnumerator()|where-Object{$_.Value.getType().Name -like "*Date*"}).Name
    foreach($key in $dateProperties)
    {

        $testResult=$true
        if($null -eq $CurrentValues.$key -xor $null -eq $PSBoundParameters.$key)
        {
            $testResult=$false
            break;
        }

        if($null -ne $CurrentValues.$key -and $null -ne $PSBoundParameters.$key)
        {
            $sourceDate= ([DateTime]($CurrentValues.$key.DateTime)).ToLocalTime()
            write-verbose($sourceDate.toString())
            write-verbose($PSBoundParameters.$key)
            write-verbose($targetDate.toString())
...
function Export-TargetResource
{
    ...
    try
    {
        ...
        foreach ($config in $getValue)
        {
            ...
            {
                $complexTypeStringResult = Get-M365DSCDRGComplexTypeToString -ComplexObject $Results.RolloutSettings -CIMInstanceName MicrosoftGraphwindowsUpdateRolloutSettings
