...
function Get-TargetResource
{
    ...
    Write-Verbose -Message "Getting configuration for Teams Unassigned Number Treatment $Identity"
    ...
    try
    {
        if (-not $Script:exportedInstance -or $Script:exportedInstance.Identity -ne $Identity)
        {
            ...
            $instance = Get-CsTeamsUnassignedNumberTreatment -Identity $Identity -ErrorAction SilentlyContinue
        }
        else
        {
...
function Set-TargetResource
{
    ...
    Write-Verbose -Message "Setting configuration for Teams Unassigned Number Treatment $Identity"
    ...
    {
        Write-Verbose -Message "Updating the Teams Unassigned Number Treatment with Identity {$Identity}"
        $UpdateParameters = Remove-M365DSCAuthenticationParameter -BoundParameters $PSBoundParameters
        Set-CsTeamsUnassignedNumberTreatment @UpdateParameters | Out-Null
    }
    elseif ($Ensure -eq 'Absent' -and $currentInstance.Ensure -eq 'Present')
    {
        Write-Verbose -Message "Removing the Teams Unassigned Number Treatment with Identity {$Identity}"
