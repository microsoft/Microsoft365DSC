Confirm-M365DSCModuleDependency -ModuleName 'MSFT_IntuneTrustedRootCertificateAndroidEnterprise'
...
function Get-TargetResource
{
    ...
    Write-Verbose -Message "Getting configuration of the Intune Trusted Root Certificate Policy for Android with Id {$Id} and DisplayName {$DisplayName}"
    ...
    try
    {
        if (-not $Script:exportedInstance -or $Script:exportedInstance.DisplayName -ne $DisplayName)
        {
            ...
            if ($null -eq $getValue)
            {
                ...
                -FilterScript { `
                    $_.AdditionalProperties.'@odata.type' -eq '#microsoft.graph.androidTrustedRootCertificate' `
                }
            }
            #endregion
            ...
            if ($null -eq $getValue)
            {
                Write-Verbose -Message "No Intune Trusted Root Certificate Policy for Android with Id {$id} was found"
                return $nullResult
            }
        }
        else
...
function Set-TargetResource
{
    ...
    Write-Verbose -Message "Setting configuration of the Intune Trusted Root Certificate Policy for Android with Id {$Id} and DisplayName {$DisplayName}"
...
function Get-M365DSCAdditionalProperties
{
    ...
    $results = @{'@odata.type' = '#microsoft.graph.androidTrustedRootCertificate' }
