function Get-TargetResource
{
    ...
    Write-Verbose -Message "Getting configuration of Microsoft Place Room for $Identity"
    ...
    try
    {
        ...
        if ($null -eq $room)
        {
            Write-Verbose -Message "Microsoft Place Room with Identity {$Identity} was not found"
            return $nullReturn
        }

        # Verify this is a room type place
        if ($room.'@odata.type' -ne '#microsoft.graph.room' -or 
            ($room.PlaceType -and $room.PlaceType -ne 'Room'))
        {
            Write-Verbose -Message "Place with Identity {$Identity} is not a room"
            return $nullReturn
        }

        Write-Verbose -Message "Found Microsoft Place Room with Identity {$Identity}"

        $result = @{
            Identity              = $room.id
            DisplayName           = $room.displayName
            Address               = $room.address.street
            ...
            Phone                 = $room.phone
            GeoCoordinates        = if ($room.geoCoordinates) { "$($room.geoCoordinates.latitude),$($room.geoCoordinates.longitude)" } else { $null }
            Capacity              = $room.capacity
            BuildingId            = $room.building
            Floor                 = $room.floor
            FloorLabel            = $room.floorLabel
            Label                 = $room.label
            IsWheelChairAccessible = [Boolean]$room.isWheelChairAccessible
            AudioDeviceName       = $room.audioDeviceName
            VideoDeviceName       = $room.videoDeviceName
            DisplayDeviceName     = $room.displayDeviceName
            MTREnabled            = [Boolean]$room.mtrEnabled
            Tags                  = $room.tags
            Ensure                = 'Present'
...
function Set-TargetResource
{
    ...
    Write-Verbose -Message "Setting configuration of Microsoft Place Room for $Identity"
    ...
    {
        $requestBody.audioDeviceName = $AudioDeviceName
    }

    if (-not [String]::IsNullOrEmpty($VideoDeviceName))
    {
        $requestBody.videoDeviceName = $VideoDeviceName
    }

    if (-not [String]::IsNullOrEmpty($DisplayDeviceName))
    {
        $requestBody.displayDeviceName = $DisplayDeviceName
    }

    if ($PSBoundParameters.ContainsKey('MTREnabled'))
    {
        $requestBody.mtrEnabled = $MTREnabled
    }
    ...
    try
    {
        ...
        {
            Write-Verbose -Message "Creating Microsoft Place Room {$Identity}"
            $requestBody.'@odata.type' = '#microsoft.graph.room'
            $requestBody.placeType = 'Room'
...
function Test-TargetResource
{
    ...
    Write-Verbose -Message "Testing configuration of Microsoft Place Room for $Identity"
...
function Export-TargetResource
{
    ...
    try
    {
        ...
        {
            Write-Host "    |---[$i/$($rooms.value.Count)] $($room.displayName)" -NoNewline
