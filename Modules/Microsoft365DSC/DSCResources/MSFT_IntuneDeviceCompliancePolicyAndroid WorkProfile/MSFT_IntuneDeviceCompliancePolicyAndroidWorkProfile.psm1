...
"@
    ...
    if ($Ensure -eq 'Present' -and $currentDeviceAndroidPolicy.Ensure -eq 'Absent')
    {
        ...
        New-DeviceManagement_DeviceCompliancePolicies -androidWorkProfileCompliancePolicy @PSBoundParameters -scheduledActionsForRule $jsonObject
    }
    elseif ($Ensure -eq 'Present' -and $currentDeviceAndroidPolicy.Ensure -eq 'Present')
    {
        ...
        $configDeviceAndroidPolicy = Get-DeviceManagement_DeviceCompliancePolicies `
            ...
            -FilterScript { ($_.deviceCompliancePolicyODataType) -eq 'microsoft.graph.androidWorkProfileCompliancePolicy' -and `
            $_.displayName -eq $($DisplayName) }
        Update-DeviceManagement_DeviceCompliancePolicies -androidWorkProfileCompliancePolicy `
...
function Export-TargetResource
{
    ...
    try
    {
        ...
        foreach ($configDeviceAndroidPolicy in $configDeviceAndroidPolicies)
        {
            ...
            $content += "        IntuneAndroidWorkProfileDeviceCompliancePolicy " + (New-Guid).ToString() + "`r`n"
            $content += "        {`r`n"
