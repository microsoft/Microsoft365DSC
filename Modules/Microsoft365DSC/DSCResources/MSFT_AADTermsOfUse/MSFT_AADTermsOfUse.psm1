function Get-TargetResource
{
    ...
    try
    {
        ...
        if($currentFiles)
        {
            ...
            foreach($myFile in $results.files)
            {
    ...
    if ($Ensure -eq 'Present' -and $currentInstance.Ensure -eq 'Absent')
    {
        ...
        foreach($myFile in $Files)
        {
            ...
            if($myFile.Versions.count -gt 0)
            {
                ...
                $hashFile.add('fileData', @{'data'= $myData.FileData.Data})
            }
            if(-not ([String]::IsNullOrEmpty($hashFile.fileData.data)))
            {
...
function Test-TargetResource
{
    ...
    foreach($key in $PSBoundParameters.Keys)
    {
        if($PSBoundParameters[$key].getType().Name -like "*CimInstance*")
        {
            ...
            if(-Not $testResult)
            {
                Write-Verbose -Message "Configuration drifted in key: $key"
                $testResult=$false
                break;
            }
...
function Export-TargetResource
{
    ...
    try
    {
        ...
        if ($Results.Files)
        {
            ...
            foreach($myFile in $Results.Files)
            {
                Write-Host "        |---[$iLocalizations/$($Results.Files.count)] Extracting Localization $($myFile.language) " -NoNewline
               if($myFile.Versions)
               {
                    ...
                    $iVersions=1
                   foreach ($version in $myfile.Versions)
                   {
                        Write-Host "            |---[$iVersions/$($myfile.Versions.count)] Extracting Version $($version.id) " -NoNewline
