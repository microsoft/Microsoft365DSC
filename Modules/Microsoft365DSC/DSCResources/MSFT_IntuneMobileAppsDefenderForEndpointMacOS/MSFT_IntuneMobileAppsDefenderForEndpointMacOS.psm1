Confirm-M365DSCModuleDependency -ModuleName 'MSFT_IntuneMobileAppsDefenderForEndpointMacOS'
...
function Get-TargetResource
{
    ...
    Write-Verbose -Message "Getting configuration for the Intune Mobile Apps Defender For Endpoint for macOS with Id {$Id} and DisplayName {$DisplayName}"
    ...
    try
    {
        if (-not $Script:exportedInstance -or $Script:exportedInstance.DisplayName -ne $DisplayName)
        {
            ...
            if ($null -eq $getValue)
            {
                Write-Verbose -Message "Could not find an Intune Mobile Apps Defender For Endpoint for macOS with Id {$Id}"

                if (-not [System.String]::IsNullOrEmpty($DisplayName))
                {
                    $getValue = Get-MgBetaDeviceAppManagementMobileApp `
                        -Filter "DisplayName eq '$($DisplayName -replace "'", "''")' and isof('microsoft.graph.macOSMicrosoftDefenderApp')" `
            ...
            if ($null -eq $getValue)
            {
                Write-Verbose -Message "Could not find an Intune Mobile Apps Defender For Endpoint for macOS with DisplayName {$DisplayName}."
                return $nullResult
            }
        ...
        $Id = $getValue.Id
        Write-Verbose -Message "An Intune Mobile Apps Defender For Endpoint for macOS with Id {$Id} and DisplayName {$DisplayName} was found"
...
function Set-TargetResource
{
    ...
    Write-Verbose -Message "Setting configuration of the Intune Mobile Apps Defender For Endpoint for macOS with Id {$Id} and DisplayName {$DisplayName}"
    ...
    {
        Write-Verbose -Message "Creating an Intune Mobile Apps Defender For Endpoint for macOS with DisplayName {$DisplayName}"
...
function Test-TargetResource
{
    ...
    Write-Verbose -Message "Testing configuration of the Intune Mobile Apps Defender For Endpoint for macOS with Id {$Id} and DisplayName {$DisplayName}"
