...
$Global:DscHelper = New-M365DscUnitTestHelper -StubModule $CmdletModule `
    -DscResource "IntuneMobileAppsMicrosoftEdge" -GenericStubModule $GenericStubPath
Describe -Name $Global:DscHelper.DescribeHeader -Fixture {
    InModuleScope -ModuleName $Global:DscHelper.ModuleName -ScriptBlock {
        ...
        # Test contexts
        Context -Name "The IntuneMobileAppsMicrosoftEdge should exist but it DOES NOT" -Fixture {
            BeforeAll {
                $testParams = @{
        ...
        Context -Name "The IntuneMobileAppsMicrosoftEdge exists but it SHOULD NOT" -Fixture {
            BeforeAll {
                $testParams = @{
                    TargetPlatform = "windows"
                    Categories = [CimInstance[]]@((New-CimInstance -ClassName MSFT_DeviceManagementMobileAppCategory -Property @{
        ...
        Context -Name "The IntuneMobileAppsMicrosoftEdge Exists and Values are already in the desired state" -Fixture {
            BeforeAll {
                $testParams = @{
                    TargetPlatform = "windows"
                    Categories = [CimInstance[]]@((New-CimInstance -ClassName MSFT_DeviceManagementMobileAppCategory -Property @{
        ...
        Context -Name "The IntuneMobileAppsMicrosoftEdge exists and values are NOT in the desired state" -Fixture {
            BeforeAll {
                $testParams = @{
                    TargetPlatform = "windows"
                    Categories = [CimInstance[]]@((New-CimInstance -ClassName MSFT_DeviceManagementMobileAppCategory -Property @{
