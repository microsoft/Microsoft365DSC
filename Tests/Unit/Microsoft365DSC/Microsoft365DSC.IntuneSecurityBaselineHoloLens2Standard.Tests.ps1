...
$Global:DscHelper = New-M365DscUnitTestHelper -StubModule $CmdletModule `
    -DscResource "IntuneSecurityBaselineHoloLens2Standard" -GenericStubModule $GenericStubPath
Describe -Name $Global:DscHelper.DescribeHeader -Fixture {
    InModuleScope -ModuleName $Global:DscHelper.ModuleName -ScriptBlock {
        ...
        BeforeAll {
            ...
            Mock -CommandName Get-MgBetaDeviceManagementConfigurationPolicy -MockWith {
                return @{
                    ...
                    TemplateReference = @{
                        TemplateId = '5d030c29-ea8d-4751-ba0d-5e3de7a84cd3_1'
                    }
                }
            }
        ...
        # Test contexts
        Context -Name "The IntuneSecurityBaselineHoloLens2Standard should exist but it DOES NOT" -Fixture {
            BeforeAll {
                $testParams = @{
        ...
        Context -Name "The IntuneSecurityBaselineHoloLens2Standard exists but it SHOULD NOT" -Fixture {
            BeforeAll {
                $testParams = @{
                    AADGroupMembershipCacheValidityInDays = 7;
                    Assignments = [CimInstance[]]@(
        ...
        Context -Name "The IntuneSecurityBaselineHoloLens2Standard Exists and Values are already in the desired state" -Fixture {
            BeforeAll {
                $testParams = @{
                    AADGroupMembershipCacheValidityInDays = 7;
                    Assignments = [CimInstance[]]@(
        ...
        Context -Name "The IntuneSecurityBaselineHoloLens2Standard exists and values are NOT in the desired state" -Fixture {
            BeforeAll {
                $testParams = @{
