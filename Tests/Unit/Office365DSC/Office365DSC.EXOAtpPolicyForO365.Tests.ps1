    ...
    InModuleScope -ModuleName $Global:DscHelper.ModuleName -ScriptBlock {
        ...
        Context -Name 'Test Fails When the Ensure Absent is specified' -Fixture {
            ...
            It 'Should throw error from the Get method' {
                { Get-TargetResource @testParams } | Should Throw "EXOAtpPolicyForO365 configurations MUST specify Ensure value of 'Present'"
            }

            It 'Should throw error from the Set method' {
                { Set-TargetResource @testParams } | Should Throw "EXOAtpPolicyForO365 configurations MUST specify Ensure value of 'Present'"
            }

            It 'Should throw error from the Test method' {
                { Test-TargetResource @testParams } | Should Throw "EXOAtpPolicyForO365 configurations MUST specify Ensure value of 'Present'"
            }
        }
